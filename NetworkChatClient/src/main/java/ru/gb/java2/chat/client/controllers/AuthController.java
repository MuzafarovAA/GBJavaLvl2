...
public class AuthController {
    ...
    public void initMessageHandler() {
        Network.getInstance().waitMessages(message -> {
            ...
            } else if (message.startsWith(WRONG_USERNAME_COMMAND)) {
                Platform.runLater(() -> {
                    clientChat.showAuthErrorDialog(INVALID_CREDENTIALS, "Пользователя с таким логином и паролем не существует!");
                });
            } else if (message.startsWith(ALREADY_EXIST_USERNAME_COMMAND)) {
                Platform.runLater(() -> {
                    clientChat.showAuthErrorDialog(INVALID_CREDENTIALS, "Такой пользователь уже зарегистрирован");
                });
            }
        });
    }
